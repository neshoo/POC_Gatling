...
class RequestToCBS extends Simulation {
  ...
  val scn = scenario("Request To Database").feed(uuidFeeder)
    .exec(http("Request To Database Request")
      .post("/ktb/rest/redhat/v1/hello-world/cbs-call").body(StringBody("""{"accountNO": 6700181844}""")).asJson)

      setUp(scn.inject(atOnceUsers(1))).protocols(httpConf.headers(headers))
